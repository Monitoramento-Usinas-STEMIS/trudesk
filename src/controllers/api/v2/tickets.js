...
ticketsV2.single = async function (req, res) {
    ...
    try {
        ...
        if (req.user.role.isAdmin || req.user.role.isAgent) {
            ...
            const groupIds = userGroups.map(m => m._id.toString());
        ...
        logger.error('Error fetching single ticket:', err);
        return apiUtils.sendApiError(res, 500, err.message);
    }
};
...
ticketsV2.transferToThirdParty = async (req, res) => {
    ...
    try {
        ...
        logger.error('Error transferring ticket to third party:', error);
...
ticketsV2.info.types = async (req, res) => {
    try {
        ...
        logger.error('Error fetching ticket types:', err);
        return apiUtils.sendApiError(res, 500, err.message);
    }
};

...
async function getWarrantyTypeId() {
  try {
    ...
    return warrantyType ? warrantyType._id : null;
  } catch (err) {
    winston.warn('Error fetching warranty type ID:', err);
    return null;
  }
}
...
async function getNonWarrantyTypeIds() {
  try {
    const nonWarrantyTypes = await ticketTypeSchema.find({ name: { $ne: 'Garantia' } }).select('_id');
    return nonWarrantyTypes.map(type => type._id);
  } catch (err) {
    winston.warn('Error fetching non-warranty type IDs:', err);
    return [];
  }
}
...
ticketsController.getByStatus = async function (req, res, next) {
  try {
    const url = require('url');
    let page = req.params.page;
    if (_.isUndefined(page)) page = 0;

    const nonWarrantyTypeIds = await getNonWarrantyTypeIds();
...
ticketsController.getActive = async function (req, res, next) {
  try {
    let page = req.params.page;
    if (_.isUndefined(page)) page = 0;
...
ticketsController.getAssigned = async function (req, res, next) {
  try {
    let page = req.params.page;
    if (_.isUndefined(page)) page = 0;
...
ticketsController.getUnassigned = async function (req, res, next) {
  try {
    let page = req.params.page;
    if (_.isUndefined(page)) page = 0;
...
ticketsController.filter = async function (req, res, next) {
  try {
    ...
    if (!_.isUndefined(errorTypes)) errorTypes = xss(errorTypes);
    if (!_.isUndefined(errorTypes) && !_.isArray(errorTypes)) errorTypes = [errorTypes];
...
ticketsController.getWarrantyByStatus = async function (req, res, next) {
  try {
    const url = require('url');
    let page = req.params.page;
    if (_.isUndefined(page)) page = 0;

    const warrantyTypeId = await getWarrantyTypeId();
    if (!warrantyTypeId) {
      winston.warn('Warranty type "Garantia" not found');
      return handleError(res, { status: 500, message: 'Warranty type not configured' });
    }

    const processor = {};
    processor.title = 'Warranty Tickets';
    processor.nav = 'warranty';
    processor.subnav = 'warranty-';
...
ticketsController.getWarrantyActive = async function (req, res, next) {
  try {
    let page = req.params.page;
    if (_.isUndefined(page)) page = 0;
    ...
    processor.title = 'Warranty Tickets';
    processor.nav = 'warranty';
    processor.subnav = 'warranty-active';
    processor.renderpage = 'warranty';
    processor.pagetype = 'active';
    processor.object = {
      limit: 50,
...
ticketsController.getWarrantyAssigned = async function (req, res, next) {
  try {
    let page = req.params.page;
    if (_.isUndefined(page)) page = 0;
...
ticketsController.getWarrantyUnassigned = async function (req, res, next) {
  try {
    let page = req.params.page;
    if (_.isUndefined(page)) page = 0;
    ...
    processor.title = 'Warranty Tickets';
    processor.nav = 'warranty';
    processor.subnav = 'warranty-unassigned';
    processor.renderpage = 'warranty';
    processor.pagetype = 'unassigned';
    processor.object = {
      limit: 50,
...
ticketsController.warrantyFilter = async function (req, res, next) {
  try {
    let page = req.query.page;
    if (_.isUndefined(page)) page = 0;
